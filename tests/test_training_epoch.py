...
def test_run_loss_epoch_skips_non_finite_batches_without_poisoning_weights():
    model = _TinyModel()
    optimizer = torch.optim.SGD(model.parameters(), lr=0.1)
    rng = np.random.default_rng(42)
    edge_indices = np.arange(4, dtype=np.int64)

    def _loss_fn(batch_idx: np.ndarray) -> torch.Tensor:
        if int(batch_idx[0]) == 0:
    ...
    assert not torch.allclose(before, after)
    assert metrics["loss"] >= 0.0 or np.isnan(metrics["loss"])

"""CLI launcher for GraphMixer temporal link prediction on stream graphs.
...
YADISK_EXPERIMENTS_BASE = "orbitaal_processed/experiments/exp_007_graphmixer"
...
def run_experiment(args):
    """Run a full GraphMixer experiment: load data, train, evaluate, upload."""
    total_start = time.time()

    parquet_name = Path(args.parquet_path).stem
    exp_name = f"graphmixer_{parquet_name}"
...
def main():
    parser = argparse.ArgumentParser(
        description="GraphMixer temporal link prediction on stream graphs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-mixer-layers", type=int, default=2,
        help="Number of MLP-Mixer blocks in LinkEncoder (1-3).",
    )
    ...
    parser.add_argument(
        ...
        help="Dimension of query-node own features (0 = NodeEncoder disabled). "
             "Not recommended for stream graph format (node features are zeros).",
    )

...
def main():
    ...
    train_cmd = (
        f"YADISK_TOKEN=\"...\" PYTHONPATH=. python src/models/HyperEvent/hyperevent_launcher.py \\\n"
        f"    --parquet-path {args.parquet_path} --epochs 50 \\\n"
    )
    for key, value in best.params.items():
        flag = key.replace("_", "-")
        train_cmd += f"    --{flag} {value} \\\n"
    train_cmd += "    --output /tmp/hyperevent_results 2>&1 | tee /tmp/hyperevent_train.log"

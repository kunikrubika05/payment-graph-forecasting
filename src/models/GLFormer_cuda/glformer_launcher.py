"""CLI launcher for GLFormer with CUDA-accelerated temporal sampling.

Usage:
    YADISK_TOKEN="..." PYTHONPATH=. python src/models/GLFormer_cuda/glformer_launcher.py \
        --parquet-path stream_graph/2020-06-01_2020-08-31.parquet \
        --edge-feat-dim 2 --epochs 10

Same CLI interface as GLFormer/glformer_launcher.py, but uses
TemporalGraphSampler (CUDA backend) for neighbor sampling.
"""
...
YADISK_EXPERIMENTS_BASE = "orbitaal_processed/experiments/exp_006_glformer_cuda"
...
def run_experiment(args):
    """Run a full GLFormer CUDA experiment: load data, train, evaluate, upload."""
    total_start = time.time()

    parquet_name = Path(args.parquet_path).stem
    exp_name = f"glformer_cuda_{parquet_name}"
    ...
    logger.info("=" * 60)
    logger.info("GLFormer CUDA Experiment: %s", exp_name)
    ...
    logger.info("Step 2: Training GLFormer (CUDA sampling)...")
    train_start = time.time()
    model, history = train_glformer_cuda(
        data=data,
    ...
    logger.info("  Sampling backend: %s", args.sampling_backend)
...
def main():
    parser = argparse.ArgumentParser(
        description="GLFormer with CUDA-accelerated temporal sampling"
    )
    parser.add_argument(
        "--parquet-path",
        type=str,
    ...
    parser.add_argument(
        "--output", type=str, default="/tmp/glformer_cuda_results",
    )
    parser.add_argument(
        "--sampling-backend", type=str, default="auto",
        choices=["auto", "cuda", "cpp", "python"],
        help="Backend for TemporalGraphSampler (default: auto = CUDA if available)",
    )
